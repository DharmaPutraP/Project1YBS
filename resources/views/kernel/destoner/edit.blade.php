        ...
        <form action="{{ route('kernel.destoner.update', $kernelDestoner->id) }}" method="POST" class="space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    @else
                        <input type="text" name="operator" id="operator" value="{{ old('operator', $kernelDestoner->operator) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm @error('operator') border-red-400 bg-red-50 @enderror">
                    @endif
                ...
                <div>
                    ...
                    <input type="text" name="sampel_boy" id="sampel_boy" value="{{ old('sampel_boy', $kernelDestoner->sampel_boy) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm @error('sampel_boy') border-red-400 bg-red-50 @enderror">
                ...
                <div>
                    ...
                    <input type="number" step="0.0001" name="berat_sampel" id="berat_sampel" value="{{ old('berat_sampel', $kernelDestoner->berat_sampel) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm @error('berat_sampel') border-red-400 bg-red-50 @enderror">
                    @error('berat_sampel')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="time" class="block text-sm font-medium text-gray-700 mb-2">Time (detik) <span class="text-red-500">*</span></label>
                    <input type="number" step="0.0001" name="time" id="time" value="{{ old('time', $kernelDestoner->time) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm @error('time') border-red-400 bg-red-50 @enderror">
                    @error('time')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="berat_nut" class="block text-sm font-medium text-gray-700 mb-2">Berat Nut (gram) <span class="text-red-500">*</span></label>
                    <input type="number" step="0.0001" name="berat_nut" id="berat_nut" value="{{ old('berat_nut', $kernelDestoner->berat_nut) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm @error('berat_nut') border-red-400 bg-red-50 @enderror">
                    @error('berat_nut')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="berat_kernel" class="block text-sm font-medium text-gray-700 mb-2">Berat Kernel (gram) <span class="text-red-500">*</span></label>
                    <input type="number" step="0.0001" name="berat_kernel" id="berat_kernel" value="{{ old('berat_kernel', $kernelDestoner->berat_kernel) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm @error('berat_kernel') border-red-400 bg-red-50 @enderror">
