                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        @foreach($group['items'] as $item)
                            @php
                                $kode = $item['kode'];
                            @endphp
                            <div class="border rounded-lg p-4 bg-white/80 shadow-sm space-y-3 {{ $cardClass }}">
                                ...
                                <div class="grid grid-cols-1 gap-3">
                                    ...
                                    <div>
                                        ...
                                        @if(!empty($operatorOptions))
                                            <select name="rows[{{ $kode }}][operator]" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                        ...
                                        @else
                                            <input type="text" name="rows[{{ $kode }}][operator]" value="{{ old("rows.$kode.operator") }}" placeholder="Nama operator" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="number" step="0.0001" name="rows[{{ $kode }}][berat_sampel]" value="{{ old("rows.$kode.berat_sampel") }}" placeholder="0.0000" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-1">Berat Dirty (gram)</label>
                                    <input type="number" step="0.0001" name="rows[{{ $kode }}][berat_dirty]" value="{{ old("rows.$kode.berat_dirty") }}" placeholder="0.0000" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-1">Moist (%)</label>
                                    <input type="number" step="0.0001" name="rows[{{ $kode }}][moist_percent]" value="{{ old("rows.$kode.moist_percent") }}" placeholder="0.0000" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
                                </div>
                            </div>
                        @endforeach
                    </div>
