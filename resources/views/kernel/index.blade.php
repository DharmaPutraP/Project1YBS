        ...
        <div class="mb-4 md:mb-6 bg-gray-50 rounded-lg p-3 md:p-4 border border-gray-200">
            ...
            <div class="mt-3 text-xs md:text-sm text-gray-600 flex flex-wrap items-center gap-2">
                ...
                @if($startDate == $endDate)
                    <span class="inline-flex items-center px-2 md:px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800 whitespace-nowrap">
                        📅 {{ \Carbon\Carbon::parse($startDate)->format('d M Y') }}
                    </span>
                @else
                    <span class="inline-flex items-center px-2 md:px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
                        📅 {{ \Carbon\Carbon::parse($startDate)->format('d M Y') }} -
                        {{ \Carbon\Carbon::parse($endDate)->format('d M Y') }}
                    </span>
                @endif
                @if(request('kode'))
                    <span class="inline-flex items-center px-2 md:px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800 whitespace-nowrap">
                        🔖 Kode: {{ request('kode') }}
                    </span>
                @else
                    <span class="inline-flex items-center px-2 md:px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-700 whitespace-nowrap">
                        🔖 Semua Kode
                    </span>
                @endif
            </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-blue-50">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Berat Sampel (g)</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Nut Utuh-Nut (g)</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Nut Utuh-Kernel (g)</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Nut Pecah-Nut (g)</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Nut Pecah-Kernel (g)</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Kernel Utuh (g)</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Kernel Pecah (g)</th>
                                    <th class="px-4 py-3 text-center text-xs font-medium text-purple-700 uppercase tracking-wider bg-purple-100">KTS Nut Utuh</th>
                                    <th class="px-4 py-3 text-center text-xs font-medium text-purple-700 uppercase tracking-wider bg-purple-100">KTS Nut Pecah</th>
                                    <th class="px-4 py-3 text-center text-xs font-medium text-purple-700 uppercase tracking-wider bg-purple-100">Kernel Utuh/Sampel</th>
                                    <th class="px-4 py-3 text-center text-xs font-medium text-purple-700 uppercase tracking-wider bg-purple-100">Kernel Pecah/Sampel</th>
                                    <th class="px-4 py-3 text-center text-xs font-medium text-red-700 uppercase tracking-wider bg-red-50">Kernel Losses</th>
                                    <th class="px-4 py-3 text-center text-xs font-medium text-orange-700 uppercase tracking-wider bg-orange-50">Limit</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-700 uppercase tracking-wider">Input By</th>
                                ...
                                @foreach ($kernelCalculations as $calc)
                                    ...
                                    <tr class="hover:bg-blue-50">
                                        ...
                                        <td class="px-4 py-3 whitespace-nowrap text-center">
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold {{ $lossBadge }}">
                                        ...
                                        <td class="px-4 py-3 whitespace-nowrap text-sm font-medium">
                                            @can('delete kernel losses')
                                                <form action="{{ route('kernel.calculations.destroy', $calc->id) }}" method="POST"
                                                    class="inline delete-form" data-item-name="Data {{ $calc->kode ?? 'ini' }}">
                                                    @csrf
                                                    @method('DELETE')
