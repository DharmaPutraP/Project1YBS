<x-layouts.app title="Laporan Destoner">
    ...
    <x-ui.card title="Tabel Data Laporan Destoner">
        ...
        <div class="relative overflow-x-auto overflow-y-auto max-h-[600px] border rounded-lg">
            <table class="min-w-[2200px] text-xs text-gray-700 divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($rows as $row)
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="border px-3 py-2 text-right">{{ number_format((float) ($row->konversi_kg ?? 0), 6) }}</td>
                            <td class="border px-3 py-2 text-right">{{ number_format((float) ($row->time ?? 0), 4) }}</td>
                            <td class="border px-3 py-2 text-right">{{ number_format((float) ($row->rasio_jam_kg ?? 0), 6) }}</td>
                            <td class="border px-3 py-2 text-right">{{ number_format((float) ($row->berat_nut ?? 0), 4) }}</td>
                            <td class="border px-3 py-2 text-center bg-purple-50">{{ number_format((float) ($row->persen_nut ?? 0), 4) }}</td>
                            <td class="border px-3 py-2 text-right">{{ number_format((float) ($row->berat_kernel ?? 0), 4) }}</td>
                            <td class="border px-3 py-2 text-center bg-purple-50">{{ number_format((float) ($row->persen_kernel ?? 0), 4) }}</td>
                            ...
                            <td class="border px-3 py-2 text-right bg-orange-50">{{ number_format((float) ($row->loss_kernel_jam ?? 0), 6) }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="22" class="px-4 py-16 text-center text-sm text-gray-400">Belum ada data laporan Destoner untuk periode ini.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
