<x-layouts.app title="Laporan Destoner">
    ...
    <x-ui.card title="Tabel Data Laporan Destoner">
        ...
        <style>
            .tbl-ds { border-collapse: separate; border-spacing: 0; }
            .tbl-ds thead tr.grp th { font-size:.65rem;font-weight:700;letter-spacing:.07em;text-transform:uppercase;padding:6px 8px;border-bottom:none; }
            .tbl-ds thead tr.sub th { font-size:.65rem;font-weight:600;padding:7px 8px;border-top:1px solid rgba(0,0,0,.08);white-space:nowrap;text-transform:uppercase; }
            .tbl-ds .fz { position:sticky;background:#fff; }
            .tbl-ds .fz-last { box-shadow:4px 0 8px -2px rgba(0,0,0,.13); }
            .tbl-ds tbody tr:nth-child(even) td { background:#f9fafb; }
            .tbl-ds tbody tr:nth-child(even) td.fz { background:#f9fafb!important; }
            .tbl-ds tbody tr:hover td { background:#fdf4ff!important; }
            .tbl-ds tbody tr:hover td.fz { background:#fdf4ff!important; }
        </style>
        <div class="relative overflow-x-auto overflow-y-auto max-h-[600px] border border-gray-200 rounded-xl shadow-sm">
            <table class="tbl-ds min-w-max text-xs text-gray-700">
                <thead class="sticky top-0 z-40">
                    <tr class="grp">
                        <th colspan="5" class="fz sticky left-0 z-[60] bg-fuchsia-800 text-white text-center border-r-2 border-fuchsia-400" style="min-width:380px">📋 IDENTITAS SAMPEL</th>
                        <th colspan="8" class="bg-slate-600 text-white text-center border-r border-slate-400">INFO UMUM</th>
                        <th colspan="8" class="bg-teal-700 text-white text-center border-r border-teal-400">⚗️ DATA PENGUKURAN</th>
                        <th colspan="4" class="bg-orange-700 text-white text-center">🔴 KERNEL LOSSES</th>
                    </tr>
                    <tr class="sub">
                        <th class="fz sticky left-0 z-[55] bg-fuchsia-700 text-fuchsia-100 border border-fuchsia-600/40 w-[50px] text-center" style="left:0">#</th>
                        <th class="fz sticky z-[50] bg-fuchsia-700 text-fuchsia-100 border border-fuchsia-600/40 w-[90px]" style="left:50px">BULAN</th>
                        <th class="fz sticky z-[50] bg-fuchsia-700 text-fuchsia-100 border border-fuchsia-600/40 w-[90px]" style="left:140px">TANGGAL</th>
                        <th class="fz sticky z-[50] bg-fuchsia-700 text-fuchsia-100 border border-fuchsia-600/40 w-[75px]" style="left:230px">JAM</th>
                        <th class="fz fz-last sticky z-[50] bg-fuchsia-700 text-fuchsia-100 border border-fuchsia-600/40 w-[75px]" style="left:305px">KODE</th>
                        ...
                        <th class="bg-teal-600 text-teal-100 border border-teal-500/50 text-right min-w-[100px]">KONVERSI (KG)</th>
                        <th class="bg-teal-600 text-teal-100 border border-teal-500/50 text-right min-w-[100px]">TIME (DETIK)</th>
                        <th class="bg-teal-600 text-teal-100 border border-teal-500/50 text-right min-w-[100px]">RASIO JAM/KG</th>
                        <th class="bg-teal-600 text-teal-100 border border-teal-500/50 text-right min-w-[100px]">BERAT NUT (g)</th>
                        <th class="bg-purple-600 text-purple-100 border border-purple-500/50 text-center min-w-[80px]">% NUT</th>
                        <th class="bg-teal-600 text-teal-100 border border-teal-500/50 text-right min-w-[110px]">BERAT KERNEL (g)</th>
                        <th class="bg-purple-600 text-purple-100 border border-purple-500/50 text-center min-w-[90px]">% KERNEL</th>
                        <th class="bg-orange-600 text-orange-100 border border-orange-500/50 text-center min-w-[120px]">TOTAL LOSSES KNL</th>
                        <th class="bg-orange-600 text-orange-100 border border-orange-500/50 text-right min-w-[120px]">LOSS KNL/JAM</th>
                        <th class="bg-orange-600 text-orange-100 border border-orange-500/50 text-center min-w-[110px]">LOSS KNL/TBS</th>
                        <th class="bg-orange-600 text-orange-100 border border-orange-500/50 text-center min-w-[90px]">LIMIT</th>
                    </tr>
                </thead>
                    ...
                    @forelse($rows as $row)
                        ...
                        <tr>
                            ...
                            <td class="fz fz-last border-r-2 border-fuchsia-200 px-3 py-2.5 z-[10]" style="left:305px">
                                <span class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-bold bg-fuchsia-100 text-fuchsia-800 ring-1 ring-fuchsia-300">
                                    {{ $row->kode }}
                                </span>
                            </td>
                        ...
                        <tr id="ds-detail-{{ $loop->index }}" style="display:none" class="bg-fuchsia-50/30">
                            <td colspan="23" class="px-4 py-4 border-b border-fuchsia-100">
                                <div class="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-3 text-xs">
                                    <div><p class="text-[10px] font-bold text-fuchsia-600 uppercase tracking-wider">Nama Sampel</p><p class="mt-0.5 font-medium">{{ $master->nama_sample ?? '-' }}</p></div>
                                    ...
                                    <div><p class="text-[10px] font-bold text-teal-500 uppercase tracking-wider">Berat Sampel</p><p class="mt-0.5 font-mono">{{ number_format((float)($row->berat_sampel??0),2) }} g</p></div>
                                    <div><p class="text-[10px] font-bold text-teal-500 uppercase tracking-wider">Konversi KG</p><p class="mt-0.5 font-mono">{{ number_format((float)($row->konversi_kg??0),2) }}</p></div>
                                    <div><p class="text-[10px] font-bold text-teal-500 uppercase tracking-wider">Time (det)</p><p class="mt-0.5 font-mono">{{ number_format((float)($row->time??0),2) }}</p></div>
                                    <div><p class="text-[10px] font-bold text-teal-500 uppercase tracking-wider">Rasio (jam/kg)</p><p class="mt-0.5 font-mono">{{ number_format((float)($row->rasio_jam_kg??0),2) }}</p></div>
                                    <div><p class="text-[10px] font-bold text-teal-500 uppercase tracking-wider">Berat Nut</p><p class="mt-0.5 font-mono">{{ number_format((float)($row->berat_nut??0),2) }}</p></div>
                                    <div><p class="text-[10px] font-bold text-purple-500 uppercase tracking-wider">% Nut</p><p class="mt-0.5 font-mono text-purple-700">{{ number_format((float)($row->persen_nut??0),2) }}%</p></div>
                                    <div><p class="text-[10px] font-bold text-teal-500 uppercase tracking-wider">Berat Kernel</p><p class="mt-0.5 font-mono">{{ number_format((float)($row->berat_kernel??0),2) }}</p></div>
                                    <div><p class="text-[10px] font-bold text-purple-500 uppercase tracking-wider">% Kernel</p><p class="mt-0.5 font-mono text-purple-700">{{ number_format((float)($row->persen_kernel??0),2) }}%</p></div>
                                    <div><p class="text-[10px] font-bold text-red-500 uppercase tracking-wider">Loss Kernel/Jam</p><p class="mt-0.5 font-mono">{{ number_format((float)($row->loss_kernel_jam??0),2) }}</p></div>
                                    <div><p class="text-[10px] font-bold text-red-500 uppercase tracking-wider">Loss Kernel/TBS</p><p class="mt-0.5"><span class="px-2 py-0.5 rounded-full text-xs {{ $limitBadge }}">{{ $lossTbs }}</span></p></div>
                                    <div><p class="text-[10px] font-bold text-orange-500 uppercase tracking-wider">Limit</p><p class="mt-0.5 font-medium text-orange-700">@if($limitOp && $limitVal!==null){{ match($limitOp){'lt'=>'<','ge'=>'>=','gt'=>'>',default=>'<='} }} {{ $limitVal }}@else-@endif</p></div>
                                </div>
                            </td>
                        </tr>
                    @empty
