                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($rows as $row)
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="border px-3 py-2 text-right">{{ number_format((float) ($row->sampel_setelah_kuarter ?? 0), 4) }}</td>
                            <td class="border px-3 py-2 text-right">{{ number_format((float) ($row->berat_nut_utuh ?? 0), 4) }}</td>
                            <td class="border px-3 py-2 text-right">{{ number_format((float) ($row->berat_nut_pecah ?? 0), 4) }}</td>
                            <td class="border px-3 py-2 text-right">{{ number_format((float) ($row->berat_kernel_utuh ?? 0), 4) }}</td>
                            <td class="border px-3 py-2 text-right">{{ number_format((float) ($row->berat_kernel_pecah ?? 0), 4) }}</td>
                            <td class="border px-3 py-2 text-right">{{ number_format((float) ($row->berat_cangkang ?? 0), 4) }}</td>
                            <td class="border px-3 py-2 text-right">{{ number_format((float) ($row->berat_batu ?? 0), 4) }}</td>
                            <td class="border px-3 py-2 text-right">{{ number_format((float) ($row->berat_fiber ?? 0), 4) }}</td>
                            <td class="border px-3 py-2 text-right">{{ number_format((float) ($row->berat_broken_nut ?? 0), 4) }}</td>
                            <td class="border px-3 py-2 text-right">{{ number_format((float) ($row->total_berat_nut ?? 0), 4) }}</td>
                            <td class="border px-3 py-2 text-center bg-purple-50">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold {{ $bnTnOk === null ? 'bg-gray-100 text-gray-700' : ($bnTnOk ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800') }}">
                            ...
                            <td class="border px-3 py-2 text-right">{{ number_format((float) ($row->ampere_screw ?? 0), 4) }}</td>
                            <td class="border px-3 py-2 text-right">{{ number_format((float) ($row->tekanan_hydraulic ?? 0), 4) }}</td>
                            <td class="border px-3 py-2 text-right">{{ number_format((float) ($row->kecepatan_screw ?? 0), 4) }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="27" class="px-4 py-16 text-center text-sm text-gray-400">Belum ada data laporan QWT Fibre Press untuk periode ini.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
