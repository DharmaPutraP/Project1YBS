<x-layouts.app title="Laporan Ripple Mill">
    ...
    <x-ui.card title="Tabel Data Laporan Ripple Mill">
        ...
        <style>
            ...
            .tbl-rm tbody tr:nth-child(even) td {
                background: #f9fafb;
            }

            .tbl-rm tbody tr:nth-child(even) td.fz {
                background: #f9fafb !important;
            }

            .tbl-rm tbody tr:hover td {
                background: #ecfdf5 !important;
            }

            .tbl-rm tbody tr:hover td.fz {
                background: #ecfdf5 !important;
            }
        </style>
        <div class="relative overflow-x-auto overflow-y-auto max-h-[600px] border border-gray-200 rounded-xl shadow-sm">
            <table class="tbl-rm min-w-max text-xs text-gray-700">
                <thead class="sticky top-0 z-40">
                    <tr class="sub">
                        <th class="fz sticky left-0 z-[55] !bg-emerald-700 text-emerald-100 border border-emerald-600/40 w-[50px] text-center"
                            style="left:0">#</th>
                        <th class="fz sticky z-[50] !bg-emerald-700 text-emerald-100 border border-emerald-600/40 w-[90px]"
                            style="left:50px">BULAN</th>
                        <th class="fz sticky z-[50] !bg-emerald-700 text-emerald-100 border border-emerald-600/40 w-[90px]"
                            style="left:140px">TANGGAL</th>
                        <th class="fz sticky z-[50] !bg-emerald-700 text-emerald-100 border border-emerald-600/40 w-[75px]"
                            style="left:230px">JAM</th>
                        <th class="fz fz-last sticky z-[50] !bg-emerald-700 text-emerald-100 border border-emerald-600/40 w-[75px]"
                        ...
                        <th class="bg-teal-600 text-teal-100 border border-teal-500/50 text-right min-w-[110px]">NUT
                            PECAH (g)</th>
                        <th class="bg-purple-600 text-purple-100 border border-purple-500/50 text-center min-w-[120px]">
                            NUT UTUH (%)</th>
                        <th class="bg-purple-600 text-purple-100 border border-purple-500/50 text-center min-w-[120px]">
                            NUT PECAH (%)</th>
                        <th class="bg-red-600 text-red-100 border border-red-500/50 text-center min-w-[110px]">
                            EFFICIENCY (%)</th>
                    ...
                    @forelse($rows as $row)
                        ...
                        <tr>
                            ...
                            <td class="fz fz-last border-r-2 border-emerald-200 px-3 py-2.5 z-[10]" style="left:305px">
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-bold bg-emerald-100 text-emerald-800 ring-1 ring-emerald-300">
                                    {{ $row->kode }}
                                </span>
                            </td>
                        ...
                        <tr id="rm-detail-{{ $loop->index }}" style="display:none" class="bg-emerald-50/30">
                            <td colspan="20" class="px-4 py-4 border-b border-emerald-100">
                                <div class="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-3 text-xs">
                                    <div>
                                        <p class="text-[10px] font-bold text-emerald-600 uppercase tracking-wider">Nama
                                            Sampel</p>
                                    ...
                                    <div>
                                        ...
                                        <p class="mt-0.5 font-mono">{{ number_format($beratSampel, 4) }} g</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-teal-500 uppercase tracking-wider">Nut Utuh</p>
                                        <p class="mt-0.5 font-mono">{{ number_format($beratUtuh, 4) }} g</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-teal-500 uppercase tracking-wider">Nut Pecah
                                        </p>
                                        <p class="mt-0.5 font-mono">{{ number_format($beratPecah, 4) }} g</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-purple-500 uppercase tracking-wider">% Nut Utuh
                                        </p>
                                        <p class="mt-0.5 font-mono text-purple-700">{{ number_format($pctUtuh, 4) }}%</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-purple-500 uppercase tracking-wider">% Nut
                                            Pecah</p>
                                        <p class="mt-0.5 font-mono text-purple-700">{{ number_format($pctPecah, 4) }}%</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-red-500 uppercase tracking-wider">Efisiensi (%)
                                        </p>
                                        <p class="mt-0.5"><span
                                                class="px-2 py-0.5 rounded-full text-xs {{ $isGood === null ? 'bg-gray-100 text-gray-700' : ($isGood ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800') }}">{{ number_format($efficiencyValue, 4) }}%</span>
                                        </p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-orange-500 uppercase tracking-wider">Limit</p>
                                        <p class="mt-0.5 font-medium text-orange-700">
                                            @if($limitValue !== null){{ match ($limitOperator) { 'lt' => '<', 'ge' => '>=', 'gt' => '>', default => '<='} }}
                                            {{ number_format($limitValue, 4) }}%@else-@endif
                                        </p>
                                    </div>
                                </div>
                            </td>
