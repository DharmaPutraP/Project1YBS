<x-layouts.app title="Laporan Kernel Losses">
    ...
    <x-ui.card title="Tabel Data Laporan Kernel Losses">
        ...
        <style>
            ...
            /* Zebra */
            .tbl-kernel tbody tr:nth-child(even) td {
                background-color: #f9fafb;
            }

            .tbl-kernel tbody tr:nth-child(even) td.fz {
                background-color: #f9fafb !important;
            }

            .tbl-kernel tbody tr:hover td {
                background-color: #eef2ff !important;
            }

            .tbl-kernel tbody tr:hover td.fz {
                background-color: #eef2ff !important;
            }
            ...
            <table class="tbl-kernel min-w-[2800px] text-xs text-gray-700">
                <thead class="sticky top-0 z-40">
                    ...
                    <tr class="sub">
                        {{-- Freeze 5 cols --}}
                        <th
                            class="fz sticky left-0 z-[60] !bg-violet-600 text-violet-100 border border-violet-500/50 w-[50px] text-center">
                            #</th>
                        <th class="fz sticky z-[55] !bg-violet-600 text-violet-100 border border-violet-500/50 w-[110px]"
                            style="left:50px">BULAN</th>
                        <th class="fz sticky z-[55] !bg-violet-600 text-violet-100 border border-violet-500/50 w-[110px]"
                            style="left:160px">TANGGAL</th>
                        <th class="fz sticky z-[55] !bg-violet-600 text-violet-100 border border-violet-500/50 w-[90px]"
                            style="left:270px">JAM</th>
                        <th class="fz fz-last sticky z-[55] !bg-violet-600 text-violet-100 border border-violet-500/50 w-[90px]"
                            style="left:360px">KODE</th>

                        {{-- Info Umum (NAMA SAMPEL tidak di-freeze) --}}
                        <th class="bg-slate-500 text-slate-100 border border-slate-400/50 min-w-[160px]">NAMA SAMPEL
                        </th>
                        ...
                        <th class="bg-cyan-600 text-cyan-100 border border-cyan-500/50 text-right">NUT UTUH - KERNEL
                        </th>
                        <th class="bg-cyan-600 text-cyan-100 border border-cyan-500/50 text-right">NUT PECAH - NUT</th>
                        <th class="bg-cyan-600 text-cyan-100 border border-cyan-500/50 text-right">NUT PECAH - KERNEL
                        </th>
                        ...
                        {{-- Kalkulasi --}}
                        <th class="bg-purple-600 text-purple-100 border border-purple-500/50 text-right">KTS NUT UTUH
                        </th>
                        <th class="bg-purple-600 text-purple-100 border border-purple-500/50 text-right">KTS NUT PECAH
                        </th>
                        <th class="bg-purple-600 text-purple-100 border border-purple-500/50 text-right">KERNEL
                            UTUH/SAMPEL</th>
                        <th class="bg-purple-600 text-purple-100 border border-purple-500/50 text-right">KERNEL
                            PECAH/SAMPEL</th>
                    ...
                    @forelse($calculations as $calc)
                        ...
                        <tr class="main-row">
                            ...
                            <td class="fz fz-last border-r-2 border-violet-200 px-3 py-2.5 z-[10]" style="left:360px">
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-bold bg-violet-100 text-violet-800 ring-1 ring-violet-300">
                                    {{ $calc->kode }}
                                </span>
                            </td>

                            {{-- NAMA SAMPEL (tidak di-freeze) --}}
                            <td class="border-r border-gray-100 px-3 py-2.5 whitespace-nowrap font-medium text-slate-700">
                                {{ $master->nama_sample ?? '-' }}</td>
                            {{-- Info Umum --}}
                            <td class="border-r border-gray-100 px-3 py-2.5 bg-slate-50 whitespace-nowrap">
                                {{ optional($calc->user)->name ?? '-' }}</td>
                            ...
                            <td class="border-r border-gray-100 px-3 py-2.5 bg-slate-50 text-center">
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ ($calc->pengulangan ?? false) ? 'bg-rose-100 text-rose-800' : 'bg-gray-100 text-gray-600' }}">
                                    {{ ($calc->pengulangan ?? false) ? 'Ya' : 'Tidak' }}
                                </span>
                            </td>
                            <td class="border-r border-slate-200 px-3 py-2.5 bg-slate-50 text-center">
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ ($calc->kegiatan_dispek ?? false) ? 'bg-amber-100 text-amber-800' : 'bg-gray-100 text-gray-600' }}">
                            ...
                            <td class="border-r border-cyan-200 px-3 py-2.5 text-right font-mono">
                                {{ number_format((float) ($calc->kernel_pecah ?? 0), 4) }}</td>

                            {{-- Kalkulasi --}}
                            <td
                                class="border-r border-gray-100 px-3 py-2.5 text-right font-mono bg-purple-50/40 text-purple-800">
                            ...
                            <td
                                class="border-r border-purple-200 px-3 py-2.5 text-right font-mono bg-purple-50/40 text-purple-800">
                        ...
                        <tr id="detail-{{ $loop->index }}" style="display:none" class="bg-violet-50/30">
                            <td colspan="25" class="px-4 py-4 border-b border-violet-100">
                                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-3 text-xs">
                                    <div>
                                        <p class="text-[10px] font-bold text-violet-500 uppercase tracking-wider">Nama
                                            Sampel</p>
                                    ...
                                    <div>
                                        ...
                                        <p class="mt-0.5 text-gray-700">{{ optional($calc->user)->name ?? '-' }}</p>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <p class="mt-0.5"><span
                                                class="px-2 py-0.5 rounded-full text-xs {{ ($calc->pengulangan ?? false) ? 'bg-rose-100 text-rose-800' : 'bg-gray-100 text-gray-600' }}">{{ ($calc->pengulangan ?? false) ? 'Ya' : 'Tidak' }}</span>
                                        </p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Keg.
                                            Dispatch</p>
                                        <p class="mt-0.5"><span
                                                class="px-2 py-0.5 rounded-full text-xs {{ ($calc->kegiatan_dispek ?? false) ? 'bg-amber-100 text-amber-800' : 'bg-gray-100 text-gray-600' }}">{{ ($calc->kegiatan_dispek ?? false) ? 'Ya' : 'Tidak' }}</span>
                                        </p>
                                    </div>
                                    <div
                                        class="col-span-2 sm:col-span-3 lg:col-span-4 border-t border-violet-100 pt-2 mt-1">
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-cyan-500 uppercase tracking-wider">Berat Sampel
                                        </p>
                                        <p class="mt-0.5 font-mono text-gray-800">
                                            {{ number_format((float) ($calc->berat_sampel ?? 0), 4) }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-cyan-500 uppercase tracking-wider">Nut Utuh -
                                            Nut</p>
                                        <p class="mt-0.5 font-mono text-gray-800">
                                            {{ number_format((float) ($calc->nut_utuh_nut ?? 0), 4) }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-cyan-500 uppercase tracking-wider">Nut Utuh -
                                            Kernel</p>
                                        <p class="mt-0.5 font-mono text-gray-800">
                                            {{ number_format((float) ($calc->nut_utuh_kernel ?? 0), 4) }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-cyan-500 uppercase tracking-wider">Nut Pecah -
                                            Nut</p>
                                        <p class="mt-0.5 font-mono text-gray-800">
                                            {{ number_format((float) ($calc->nut_pecah_nut ?? 0), 4) }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-cyan-500 uppercase tracking-wider">Nut Pecah -
                                            Kernel</p>
                                        <p class="mt-0.5 font-mono text-gray-800">
                                            {{ number_format((float) ($calc->nut_pecah_kernel ?? 0), 4) }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-cyan-500 uppercase tracking-wider">Kernel Utuh
                                        </p>
                                        <p class="mt-0.5 font-mono text-gray-800">
                                            {{ number_format((float) ($calc->kernel_utuh ?? 0), 4) }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-cyan-500 uppercase tracking-wider">Kernel Pecah
                                        </p>
                                        <p class="mt-0.5 font-mono text-gray-800">
                                            {{ number_format((float) ($calc->kernel_pecah ?? 0), 4) }}</p>
                                    </div>
                                    <div
                                        class="col-span-2 sm:col-span-3 lg:col-span-4 border-t border-violet-100 pt-2 mt-1">
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-purple-500 uppercase tracking-wider">KTS Nut
                                            Utuh</p>
                                        <p class="mt-0.5 font-mono text-purple-700">
                                            {{ number_format((float) ($calc->kernel_to_sampel_nut_utuh ?? 0), 4) }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-purple-500 uppercase tracking-wider">KTS Nut
                                            Pecah</p>
                                        <p class="mt-0.5 font-mono text-purple-700">
                                            {{ number_format((float) ($calc->kernel_to_sampel_nut_pecah ?? 0), 4) }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-purple-500 uppercase tracking-wider">Kernel
                                            Utuh/Sampel</p>
                                        <p class="mt-0.5 font-mono text-purple-700">
                                            {{ number_format((float) ($calc->kernel_utuh_to_sampel ?? 0), 4) }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-purple-500 uppercase tracking-wider">Kernel
                                            Pecah/Sampel</p>
                                        <p class="mt-0.5 font-mono text-purple-700">
                                            {{ number_format((float) ($calc->kernel_pecah_to_sampel ?? 0), 4) }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-red-500 uppercase tracking-wider">Kernel Losses
                                        </p>
                                        <p class="mt-0.5">@if($isExceeded === null)<span
                                        class="kbadge-na">{{ number_format($lossPercent, 4) }}%</span>@elseif($isExceeded)<span
                                                class="kbadge-bad">{{ number_format($lossPercent, 4) }}%</span>@else<span
                                                class="kbadge-ok">{{ number_format($lossPercent, 4) }}%</span>@endif</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-amber-500 uppercase tracking-wider">Limit</p>
                                        <p class="mt-0.5 font-medium text-amber-700">
                                            {{ $master ? $master->limit_label : '-' }}</p>
                                    </div>
                                </div>
                            </td>
                        </tr>
