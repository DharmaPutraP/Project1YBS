        ...
        <form action="{{ route('kernel.ripple-mill.update', $kernelRippleMill->id) }}" method="POST" class="space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    @if(!empty($operatorOptions))
                        <select name="operator" id="operator" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm @error('operator') border-red-400 bg-red-50 @enderror">
                            ...
                            @foreach($operatorOptions as $operatorName)
                                <option value="{{ $operatorName }}" {{ old('operator', $kernelRippleMill->operator) == $operatorName ? 'selected' : '' }}>{{ $operatorName }}</option>
                            @endforeach
                        </select>
                    @else
                        <input type="text" name="operator" id="operator" value="{{ old('operator', $kernelRippleMill->operator) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm @error('operator') border-red-400 bg-red-50 @enderror">
                    @endif
                ...
                <div>
                    ...
                    <input type="text" name="sampel_boy" id="sampel_boy" value="{{ old('sampel_boy', $kernelRippleMill->sampel_boy) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm @error('sampel_boy') border-red-400 bg-red-50 @enderror">
                    @error('sampel_boy')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="berat_sampel" class="block text-sm font-medium text-gray-700 mb-2">Berat Sampel <span class="text-red-500">*</span></label>
                    <input type="number" step="0.0001" name="berat_sampel" id="berat_sampel" value="{{ old('berat_sampel', $kernelRippleMill->berat_sampel) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm @error('berat_sampel') border-red-400 bg-red-50 @enderror">
                ...
                <div>
                    ...
                    <input type="number" step="0.0001" name="berat_nut_utuh" id="berat_nut_utuh" value="{{ old('berat_nut_utuh', $kernelRippleMill->berat_nut_utuh) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm @error('berat_nut_utuh') border-red-400 bg-red-50 @enderror">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="number" step="0.0001" name="berat_nut_pecah" id="berat_nut_pecah" value="{{ old('berat_nut_pecah', $kernelRippleMill->berat_nut_pecah) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm @error('berat_nut_pecah') border-red-400 bg-red-50 @enderror">
