<x-layouts.app title="Data Analisa Spintest Light Phase">
    <x-process.analisa-moisture.page-shell
        eyebrow="Analisa Moisture & Spintes"
        title="Data Analisa Spintest Light Phase"
        description="Menampilkan data tersimpan untuk periode yang dipilih."
        :input-url="route('analisa-moisture.input')"
        :filter-url="route('analisa-moisture.spintest-light-phase')"
        :export-url="route('analisa-moisture.spintest-light-phase.export')"
                                ...
                                @role('Super Admin')
                                    <td class="px-4 py-3 text-slate-700">
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('analisa-moisture.spintest-light-phase.edit', $row) }}" class="inline-flex items-center rounded-lg bg-amber-100 px-3 py-1.5 text-xs font-semibold text-amber-700 hover:bg-amber-200">Edit</a>
                                            <form method="POST" action="{{ route('analisa-moisture.spintest-light-phase.destroy', $row) }}" onsubmit="return confirm('Hapus data ini?')">
                                                @csrf
                                                @method('DELETE')
