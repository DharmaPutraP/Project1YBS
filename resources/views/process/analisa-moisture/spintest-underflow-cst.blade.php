<x-layouts.app title="Data Analisa Spintest Underflow CST">
    <x-process.analisa-moisture.page-shell
        eyebrow="Analisa Moisture & Spintes"
        title="Data Analisa Spintest Underflow CST"
        description="Menampilkan data tersimpan untuk periode yang dipilih."
        :input-url="route('analisa-moisture.input')"
        :filter-url="route('analisa-moisture.spintest-underflow-cst')"
        :export-url="route('analisa-moisture.spintest-underflow-cst.export')"
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                                ...
                                @role('Super Admin')
                                    <td class="px-4 py-3 text-slate-700">
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('analisa-moisture.spintest-underflow-cst.edit', $row) }}" class="inline-flex items-center rounded-lg bg-amber-100 px-3 py-1.5 text-xs font-semibold text-amber-700 hover:bg-amber-200">Edit</a>
                                            <form method="POST" action="{{ route('analisa-moisture.spintest-underflow-cst.destroy', $row) }}" onsubmit="return confirm('Hapus data ini?')">
                                                @csrf
                                                @method('DELETE')
                    ...
                    @empty
                            <tr>
                                <td colspan="{{ auth()->user()?->hasRole('Super Admin') ? 10 : 9 }}" class="px-4 py-8 text-center text-slate-500">Belum ada data pada periode ini.</td>
                            </tr>
                    @endforelse
                </tbody>
            </table>
