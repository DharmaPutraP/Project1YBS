<x-layouts.app title="Edit Informasi Proses Spintest">
    <div class="mb-6 flex items-center justify-between gap-3">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Edit Informasi Proses Spintest</h1>
            <p class="mt-2 text-sm text-gray-600">Ubah data proses untuk tanggal {{ $record->process_date?->format('d/m/Y') }}.</p>
        </div>
    ...
    <x-ui.card title="Form Edit Informasi Proses Spintest">
        <form action="{{ route('process.spintest.update', ['spintestProsses' => $record->id]) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="process_date" class="mb-2 block text-sm font-medium text-gray-700">
                    Tanggal <span class="text-red-500">*</span>
                </label>
                <input type="date" id="process_date" name="process_date"
                    value="{{ old('process_date', optional($record->process_date)->format('Y-m-d')) }}"
                    class="w-full rounded-lg border border-gray-300 px-4 py-2 text-sm transition focus:outline-none focus:ring-2 focus:ring-blue-500 @error('process_date') border-red-400 bg-red-50 @enderror">
                @error('process_date')
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                @if (!$visibleTeam || $visibleTeam === 'Tim 1')
                    <div class="space-y-4 rounded-xl border border-blue-200 bg-blue-50 p-5">
                        <h2 class="text-lg font-semibold text-blue-900">Tim 1</h2>

                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                                <label for="team_1_start_time" class="mb-2 block text-sm font-medium text-gray-700">
                                ...
                                <input type="time" id="team_1_start_time" name="team_1_start_time"
                                    ...
                                    class="w-full rounded-lg border border-gray-300 px-4 py-2 text-sm transition focus:outline-none focus:ring-2 focus:ring-blue-500 @error('team_1_start_time') border-red-400 bg-red-50 @enderror">
                                @error('team_1_start_time')
                                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                <label for="team_1_end_time" class="mb-2 block text-sm font-medium text-gray-700">
                                ...
                                <input type="time" id="team_1_end_time" name="team_1_end_time"
                                    ...
                                    class="w-full rounded-lg border border-gray-300 px-4 py-2 text-sm transition focus:outline-none focus:ring-2 focus:ring-blue-500 @error('team_1_end_time') border-red-400 bg-red-50 @enderror">
                ...
                @if (!$visibleTeam || $visibleTeam === 'Tim 2')
                    <div class="space-y-4 rounded-xl border border-emerald-200 bg-emerald-50 p-5">
                        <h2 class="text-lg font-semibold text-emerald-900">Tim 2</h2>

                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                                <label for="team_2_start_time" class="mb-2 block text-sm font-medium text-gray-700">
                                    Jam Mulai Proses <span class="text-red-500">*</span>
                                </label>
                                <input type="time" id="team_2_start_time" name="team_2_start_time"
                                    value="{{ old('team_2_start_time', $team2['process_start']) }}"
                                    class="w-full rounded-lg border border-gray-300 px-4 py-2 text-sm transition focus:outline-none focus:ring-2 focus:ring-emerald-500 @error('team_2_start_time') border-red-400 bg-red-50 @enderror">
                                @error('team_2_start_time')
                                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                <label for="team_2_end_time" class="mb-2 block text-sm font-medium text-gray-700">
                                    Jam Akhir Proses <span class="text-red-500">*</span>
                                </label>
                                <input type="time" id="team_2_end_time" name="team_2_end_time"
                                    value="{{ old('team_2_end_time', $team2['process_end']) }}"
                                    class="w-full rounded-lg border border-gray-300 px-4 py-2 text-sm transition focus:outline-none focus:ring-2 focus:ring-emerald-500 @error('team_2_end_time') border-red-400 bg-red-50 @enderror">
