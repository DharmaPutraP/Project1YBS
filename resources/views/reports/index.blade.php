            ...
            <table class="min-w-[4500px] text-xs text-gray-700 divide-y divide-gray-200">
                ...
                <thead class="bg-blue-50 sticky top-0 z-40">
                    <tr>
                        ...
                        <th class="sticky top-0 left-[520px] z-[55] bg-blue-50 border px-4 py-3 w-[160px]">INPUTED BY</th>
                ...
                @forelse ($calculations as $calc)
                    <tr class="hover:bg-gray-50">
                        ...
                        {{-- ===== NUMERIC (NULL → -) ===== --}}
                        <td class="border px-4 py-2 text-right">{{ $calc->cawan_kosong === null ? '-' : number_format($calc->cawan_kosong,4) }}</td>
                        <td class="border px-4 py-2 text-right">{{ $calc->berat_basah === null ? '-' : number_format($calc->berat_basah,4) }}</td>
                        <td class="border px-4 py-2 text-right">{{ $calc->total_cawan_basah === null ? '-' : number_format($calc->total_cawan_basah,4) }}</td>
                        <td class="border px-4 py-2 text-right">{{ $calc->cawan_sample_kering === null ? '-' : number_format($calc->cawan_sample_kering,4) }}</td>
                        <td class="border px-4 py-2 text-right">{{ $calc->sampel_setelah_oven === null ? '-' : number_format($calc->sampel_setelah_oven,4) }}</td>
                        <td class="border px-4 py-2 text-right">{{ $calc->labu_kosong === null ? '-' : number_format($calc->labu_kosong,4) }}</td>
                        <td class="border px-4 py-2 text-right">{{ $calc->oil_labu === null ? '-' : number_format($calc->oil_labu,4) }}</td>

                        <td class="border px-4 py-2 text-right font-bold">
                            {{ $calc->minyak === null ? '-' : number_format($calc->minyak,4) }}
                        </td>

                        <td class="border px-4 py-2 text-right">{{ $calc->moist === null ? '-' : number_format($calc->moist,2) }}</td>
                        <td class="border px-4 py-2 text-right">{{ $calc->dmwm === null ? '-' : number_format($calc->dmwm,2) }}</td>
                        <td class="border px-4 py-2 text-right">{{ $calc->olwb === null ? '-' : number_format($calc->olwb,2) }}</td>
                        <td class="border px-4 py-2 text-right">{{ $calc->limitOLWB === null ? '-' : number_format($calc->limitOLWB,2) }}</td>
                        <td class="border px-4 py-2 text-right">{{ $calc->oldb === null ? '-' : number_format($calc->oldb,2) }}</td>
                        <td class="border px-4 py-2 text-right">{{ $calc->limitOLDB === null ? '-' : number_format($calc->limitOLDB,2) }}</td>
                        <td class="border px-4 py-2 text-right font-semibold">{{ $calc->oil_losses === null ? '-' : number_format($calc->oil_losses,2) }}</td>
                        <td class="border px-4 py-2 text-right">{{ $calc->limitOL === null ? '-' : number_format($calc->limitOL,2) }}</td>
                        <td class="border px-4 py-2 text-right">{{ $calc->persen4 === null ? '-' : number_format($calc->persen4,2) }}</td>
                    </tr>
                @empty
                    <tr>
